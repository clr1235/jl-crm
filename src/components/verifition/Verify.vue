<template>
	<div v-show="showBox" class="verify-dom" :class="mode == 'pop' ? 'mask' : ''">
		<div :class="mode == 'pop' ? 'verifybox' : ''" :style="{ 'max-width': parseInt(imgSize.width) + 30 + 'px' }">
			...
			<div class="verifybox-bottom" :style="{ padding: mode == 'pop' ? '15px' : '0' }">
				...
				<keep-alive>
					<component
						v-if="componentType"
						:is="verifyTypeComponent[componentIndex]"
						ref="instanceDom"
						...
						:success-refresh="successRefresh"
						@ready="verifyReady"
...
<script lang="ts" name="Verify" setup>
/**
 * Verify 验证码组件
 * @description 分发验证码使用
 * */
import { watch, onMounted, reactive, shallowRef, ref, computed, toRefs } from 'vue'
...
interface IVerify {
	clickShow: boolean
	verifyType: undefined | number | string
...
const emit = defineEmits<{
	(e: 'verifyReady', value: any): void
	(e: 'verifySuccess', value: any): void
	(e: 'verifyError', value: any): void
}>()
const verifyReady = (value: any) => {
	emit('verifyReady', value)
}
const verifySuccess = (value: any) => {
	emit('verifySuccess', value)
}
const verifyError = (value: any) => {
	emit('verifyError', value)
}
const verifyTypeComponent = shallowRef<Array<any>>([VerifySlide, VerifyPoints])
const state: any = reactive<IVerify>({
	clickShow: false,
...
const instanceDom = ref<InstanceType<typeof VerifySlide> | InstanceType<typeof VerifyPoints> | null>(null)
// 生成 uuid
const uuid = () => {
	let s = []
...
const refresh = () => {
	if (instanceDom?.value) {
		instanceDom?.value?.refresh?.()
	}
}
const closeBox = () => {
...
const show = () => {
	props.mode == 'pop' && (state.clickShow = true)
}
const showBox = computed(() => {
	if (props.mode == 'pop') {
		return state.clickShow
	} else {
		return true
	}
})
const changeData = (params: IChangeData) => {
	state[params.dataKey] = params.data
}
watch(
	() => props.captchaType,
	(newValue) => {
		switch (newValue.toString()) {
			case 'blockPuzzle':
				...
				state.componentType = 'VerifySlide'
				break
			case 'clickWord':
				state.verifyType = ''
				state.componentIndex = 1
				state.componentType = 'VerifyPoints'
				break
		}
	},
	{ immediate: true }
...
const { verifyType, componentType, componentIndex } = toRefs(state)
defineExpose({ show })
</script>
...
.verify-dom::v-deep {
	...
	.verify-iconfont {
		font-family: 'verifyIconfont' !important;
		font-size: 16px;
		font-style: normal;
